...
# 4支路并行，无dilation
class DirectionalConv2d_nodia(nn.Module):
    def __init__(self, in_f, out_f, direction):
        super().__init__()
        self.conv = nn.Conv2d(
            in_f, out_f,
            kernel_size=3,
...
def asy_dir_conv_block_nodia(in_f, out_f, channel_list, direction1, direction2):
    '''
    参数:
        in_f: 输入通道数
        out_f: 输出通道数
    ...
    branch1_layers = []
    # branch1_layers.append(nn.ReflectionPad2d((1, 1, 1, 1)))
    branch1_layers.append(DirectionalConv2d_nodia(in_f, channel_list[0], direction1))
    # branch1_layers.append(nn.BatchNorm2d(channel_list[0], affine=bn_affine))
    branch1_layers.append(nn.BatchNorm2d(channel_list[0]))
    branch1_layers.append(nn.GELU())
    layers['branch1'] = nn.Sequential(*branch1_layers)
    ...
    branch2_layers = []
    # branch2_layers.append(nn.ReflectionPad2d((1, 1, 1, 1)))
    branch2_layers.append(DirectionalConv2d_nodia(in_f, channel_list[1], direction2))
    # branch2_layers.append(nn.BatchNorm2d(channel_list[1], affine=bn_affine))
    branch2_layers.append(nn.BatchNorm2d(channel_list[1]))
    branch2_layers.append(nn.GELU())
    layers['branch2'] = nn.Sequential(*branch2_layers)
