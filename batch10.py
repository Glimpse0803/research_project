...
class ModulatorBank(nn.Module):
    ...
    def forward(self, x, u_idx, v_idx):
        # x 形状: [B, C, H, W]，u_idx, v_idx 形状应为: [B] 的张量
        B, C, H, W = x.shape

        # 提取当前 batch 对应的所有卷积核
        k_u = self.kernels_u[u_idx]  # [B, 1, C, k, k]
        k_v = self.kernels_v[v_idx]  # [B, 1, C, k, k]

        # 在 dim=1 拼接，使每张图获得 2 个输出通道，并变更为 groups 卷积所需的核形状
        ...
        # 将 x 变形以匹配 groups 的要求
        x_reshaped = x_pad.view(1, B * C, H + 2 * self.pad, W + 2 * self.pad)

        # 使用分组卷积 (groups=B) 实现真正的并行化实例卷积
        out = F.conv2d(x_reshaped, active_kernel, groups=B)

        return out.view(B, 2, H, W)  # 恢复 batch 维度
...
def main_final_optimized():
    ...
    save_dir = f'outputs/batch10_{scene_name}'
    ...
    fixed_blocks = []
    last_head = None
    # === 新增：构建并行计算 Batch 的工具函数 ===
    device = first_target.device

    def get_batch(bs):
        actual_bs = min(bs, TOTAL_VIEWS)
        indices = torch.randperm(TOTAL_VIEWS)[:actual_bs]

        batch_u, batch_v, batch_target = [], [], []
        for idx in indices:
            u, v = view_keys[idx]
            batch_u.append(u)
            batch_v.append(v)
            batch_target.append(lf_data[(u, v)])

        b_u = torch.tensor(batch_u, dtype=torch.long, device=device)
        b_v = torch.tensor(batch_v, dtype=torch.long, device=device)
        b_target = torch.cat(batch_target, dim=0)  # [B, 3, H, W]
        b_input = net_input_saved.repeat(actual_bs, 1, 1, 1)  # [B, C, 32, 32]
        return b_input, b_u, b_v, b_target
    ...
    for step in range(TOTAL_STEPS):
        ...
        if step % 500 == 0:
            loss_val = loss.item()
            est_psnr = -10 * np.log10(loss_val)
            history_psnr.append(est_psnr)
            print(f"  Step {step}/{TOTAL_STEPS} - Avg Loss: {loss_val:.6f} | Est. PSNR: {est_psnr:.2f} dB")
    ...
    plt.title("Phase 2 Learning Curve (SA + Unfreeze + Batch10)")
