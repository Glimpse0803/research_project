    ...
    def _apply_hard(self, x):
        ...
        for k, act_layer in enumerate(self.acts):
            ...
            if len(idx) > 0:
                ...
                src = act_layer(x.index_select(1, idx))
                src = src.to(out.dtype)   
                out.index_copy_(1, idx, src)
        return out
...
class ViewInputOffset(nn.Module):
    def __init__(self, channels=64, u_views=9, v_views=9, view_kernel_w=3, view_kernel_h=3):
        super().__init__()
        # 创建 9横 9竖 的可训练小块 (1, 64, view_kernel_size, view_kernel_size)
    ...
    def forward(self, base_input, u_idx, v_idx):
        # 将小块“嵌入”到 32x32 的输入噪声中心（或者直接加法广播）
        # 这里采用加法广播，让视角信息影响全局
        offset = self.u_offsets[u_idx] + self.v_offsets[v_idx]
        # 使用 F.interpolate 将 3x3 放大到 32x32，或者直接加在 base_input 上
        # 为了保持视角切换的灵活性，我们将 offset 作用于输入
        return base_input + F.interpolate(offset, size=(32, 32), mode='bicubic')
...
class HybridBlock(nn.Module):
    def __init__(self, in_channels, out_channels=64, upsample=True, dilation=1):
        ...
        self.desc_act = ImprovedSelfAttentionAct(channels=out_channels)
...
def main_final_optimized():
    ...
    for layer_idx, (upsample, (n_search, n_refine),dilation) in enumerate(zip(upsample_configs, balanced_schedule, dilations)):
        ...
        with torch.no_grad():
            # x_f = net_input_saved
            x_f = view_offset_layer(net_input_saved,4,4)
            for blk in net.fixed_blocks: x_f = blk(x_f)
